...
export class GeminiService {
  ...
  // Initialize API key from user settings or environment variable
  static async initialize(userId: string): Promise<boolean> {
    try {
      ...
      if (error) {
        console.warn('Error fetching user settings, trying environment variable:', error);
      }
      
      // Use user setting if available, otherwise fall back to environment variable
      this.apiKey = data?.gemini_api_key || import.meta.env.VITE_GEMINI_API || null;
      
      if (this.apiKey) {
        console.log('Gemini API key loaded from:', data?.gemini_api_key ? 'user settings' : 'environment variable');
      } else {
        console.warn('No Gemini API key found in user settings or environment variables');
      }
      ...
      this.apiKey = import.meta.env.VITE_GEMINI_API || null;
      return !!this.apiKey;
    }
  }
  ...
  static async generateContent(prompt: string, context?: string): Promise<GeminiResponse> {
    ...
    try {
      ...
      return {
        content,
        confidence: 0.9, // Gemini responses are generally high confidence
        reasoning: 'Generated by Gemini 2.0 Flash'
      };
    } catch (error) {
